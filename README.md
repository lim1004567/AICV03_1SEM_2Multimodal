...
본 프로젝트는 산업 현장의 장비 상태를 실시간으로 **진단(Diagnosis)**하고, 30초 후의 상태를 **예보(Prognosis)**하는 통합 AI 솔루션입니다. 이미지와 센서 데이터를 동시에 처리하는 **Multimodal Transformer(Small)** 구조를 채택하여, 기존 논문 및 베이스라인 대비 압도적인 성능 향상을 달성했습니다. 특히, 저사양 엣지 디바이스 환경을 고려한 **INT8 양자화**를 통해 실시간성까지 확보한 '현장 맞춤형' 연구입니다.
